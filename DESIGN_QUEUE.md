...
## 80. Two ways of having to click twice, and the saves that found them

Reported together from play, and they turned out to share nothing but the
symptom. Worth recording as a pair anyway, because "I have to click it twice"
was the whole report and the two causes are in different subsystems.

### The promotion menu asked again

A unit built in a barracks arrives already promoted, so the end-of-turn chain
asks what it has learned. Answering re-opened the same menu.

The chain drained its queue **inside** `close()`, and every button in the game
answers with `close(); onDo(...)`. So the next question was asked partway
through the click that answered the last one, before the perk had been
recorded. It found the unit still owed one and asked again.

The second symptom is the one nobody could see: picking twice appended a second
perk, so the unit banked **two perks for one rank**. `owedPerks` clamps at zero,
so nothing complained. Measured on the old code, from a fixture save:

| | after one click |
|---|---|
| before | menu still up, still offering all six perks |
| after | moves on to the next question |

and the unit afterwards went from `rank 1, perks ["bloodied", "dug-in"]` to
`rank 1, perks ["bloodied"]`.

Queued callbacks now run once the handler that closed the modal has finished,
which fixes it for every `close(); onDo(...)` rather than for promotions alone.
The perk menu records its answer before dismissing; the follow-up ask for a unit
owed two waits for the close instead of trying to open underneath the menu it
came from; and the chain waits when it finds a modal up rather than giving up,
which used to mean the turn's remaining questions were never asked at all.

**This is the second bug from that seam** -- the comment in `promptPending`
describes the first, where three questions asked in a row meant only the first
ever opened. So it has tests now, against a twenty-line document stub rather
than jsdom: what is under test is *when* callbacks fire, not how HTML parses.

### The city took the second click

Nothing to do with modals. `onLeftClick` gave the tile to a unit standing on it
and the city only to the second click. Resting units were already excepted --
they are drawn as a number on the city rather than on the tile, so selecting one
would select something invisible -- but a unit with **no orders yet** was not.

Which is why it started exactly when a city finished building something: what a
city builds stands on its tile awake. Measured, nothing selected, a fresh unit
in the city:

| | first click | second click |
|---|---|---|
| before | nothing opens | city panel |
| after | city panel | |

Your own city now wins its tile outright. The cost is that selecting a unit
standing in a city is a click inside the panel rather than a click on the map,
so the panel lists **everything** standing there now, not only what is resting
out of sight -- `unitsInCity` beside `garrisonOf`, which still counts only the
hidden ones because that is what the number drawn on the city is for. A unit
with no orders reads as `ready` rather than `none`.

### Fixture saves

Reaching the barracks report by playing is twenty turns, which is why the first
attempt at it was reasoned rather than watched. `fixtures/` now holds six saved
games that load straight into a situation worth looking at: a unit owed one
perk, one owed two, a barracks about to finish, a city set to ask, a rioting
city, a full treasury.

Generated by `tests/fixtures.test.ts` and round-tripped through the real save
format, so a rules change that invalidates a scenario fails the suite rather
than leaving a save that quietly loads into something else. Byte-identical on
regeneration, so rerunning the suite does not churn them in git.
...
Asked for 2026-09-18, after the replay record. Before this, land units could not
cross water, so islands were dead ground.

**Jeremy's decisions:**
- **Scope:** a carrier and a warship for each side.
- **Advances:** existing ones only, no new tech-tree entries.
- **Map:** a new Archipelago world option; the ordinary map is unchanged.
- **Art:** drawn separately for each faction.

**The rules** (`src/sim/ships.ts`):
- **The four ships:**
  - Horde: *A Raft (Mostly)* and *The Big Angry Boat*.
  - Kingdom: *The Committee Barge* and *The Strongly Worded Frigate*.
  - Carriers come with Mapmaking. Warships come with Throwing Buddies (Horde)
    and Join the Army (Kingdom).
- **Where ships go:** water only. They are built in a coastal city, launched onto
  the water beside it, and never enter a city.
- **Passengers:**
  - A carrier holds three. They ride inside it, in `ship.cargo`, off the map, so
    one unit to a tile holds everywhere.
  - Boarding is a step onto your own ship from the shore, and it ends the unit's
    turn.
  - Passengers step ashore onto free land next to the ship, for one move. They
    cannot land straight into a foreign town.
  - If the ship sinks, everyone aboard goes down with it.
- **Fighting:** warships attack the shore without landing. Nothing on land can
  fight a ship.
- **Supply:** ships ignore the supply line.

**The Archipelago** (`ARCHIPELAGO` in worldgen):
- Noise blobs a quarter the continent's size, and 30% land.
- Each side starts on its own island, holding at least 14% of the land. Tested
  over twelve seeds.
- The ordinary world is generated byte for byte as before and saves no `world`
  field.

**The AI at sea** (`src/ai/naval.ts`, lever `NAVAL`):
- **Asking for a boat:** a land unit with somewhere to be across the water asks
  for passage. That is a settler with no site left on its island, or a soldier
  whose target is on another island.
- **Carriers:**
  - They fetch whoever is asking.
  - Settlers sail at once. Invasions wait for a full load, or up to 8 turns, so
    carriers loading together sail together.
  - They land at the beach nearest the target, anywhere on that island's coast.
    The first version looked only three tiles from the target, and every
    invasion of an inland town stalled at sea.
- **Warships** sink what they can beat, escort loaded carriers, and shell the shore.
- **Building ships:** only when someone is waiting, or when the empire is
  *stranded*. Stranded means no enemy town is known and every island it holds
  has been walked end to end. It is asked of the islands because every coast
  touches unexplored sea, so "no frontier left" never comes true.
- **Exploring:** an idle ship heads for the water that reveals the most unknown
  for the distance. "Nearest edge of the unknown" sent carriers circling one
  island for a hundred turns, staring at tiles hidden behind forest.
- **Research:** the AI takes Mapmaking when it needs a boat.

**Measured** (2026-09-18, 108 games an arm):

| arm | Horde–Kingdom | fights a game | endings (cq/dm/pt/po/ob) |
|---|---|---|---|
| continent, no navy | 57–51 | 25 / 27 | 38/8/9/27/26 |
| continent, navy | 56–52 | 25 / 27 | 38/8/9/27/26 |
| archipelago | 52–56 | 3 / 2 | 5/2/8/43/50 |

- **On the continent the navy changes one game in 108.** Everyone can walk to
  everyone, so ships are almost never asked for.
- **The archipelago is close to even, but it is a building race.** Five
  conquests in 108 games, and about three fights a game. The Portal or the
  Object decides 93 of them.
- **What watched games show:**
  - Navies are built and sink each other.
  - Invasions land, and some towns fall, but captures rarely hold.
  - Settlers seldom cross, because each island fits the AI's target number of
    towns.

**Next levers, if the archipelago should be more of a war:**
- Larger invasions: a higher `maxCarriers` and a storming party that waits
  ashore before attacking.
- A settler target that counts other islands' free land, so the AI expands
  across the water.
